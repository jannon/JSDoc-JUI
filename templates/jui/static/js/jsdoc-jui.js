...
(function($) {    
    ...
    function loadDemo(path) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                // Remove title tags
                //data = data.replace(/<iframe(.+)src=(\"|\')(.+)(\"|\')>/ig, '<iframe$1src="' + '/' + section + '/' + '$3">');
                // Change iframe src
                //data = data.replace(/<img([^<>]+)src=(\"|\')([^\"\']+)(\"|\')([^<>]+)?>/ig, '<img$1src="' + '/' + section + '/' + '$3" $5/>');
                // Change images src
                data = $.trim(data);
    ...
    function resetDemos() {
        // ($.datepicker && $.datepicker.setDefaults($.extend({
...
$(document).ready( function () {
    ...
    $('#demo-config-menu a').each(function() {
        ...
        $(this).click(function(e) {
            ...
            //Set the hash to the demo name (if the href is "ui.<wigetname>.<demoname>.html, use the demoname)
            window.location.hash = this.getAttribute('href').match((/ui\.[^\/\\]+\.([^\/\\]+)\.html/))[1]
    ...
    $('#widget-docs dd.option-type').each(function() {
        var html = $(this).text();
        var words = $.map(html.split(','), function(n) {
            return paramDescriptions[$.trim(n)] ? '<span>' + $.trim(n) + '</span>' : $.trim(n);
        });
        $(this).html(words.join(', '));
        $('span', this).click(function() {
            if(paramDescriptions[this.innerHTML])
                $('<div>' + paramDescriptions[this.innerHTML] + '</div>').dialog({
                    open : function() {
                        $(this).parent().attr('id', 'demo-dialog');
                    },
                    modal : true,
    ...
    $(".toggle-docs-private").toggle(function(e){
        $(this).text("Hide private");
        $(".private").show();
        writeCookie();
        e.preventDefault();
    }, function(e){
        $(this).text("Show private");
        $(".private").hide();
        writeCookie();
        e.preventDefault();
    });
    ...
    $('div.option-description, div.option-examples, div.event-description, div.event-examples, div.method-description, div.method-examples, .private').hide();
	...
	var cookie = readCookie( 'JUI_JSDoc' );
	if ( cookie != null ) {
		var a = cookie.split('-');
		if ( a[0] == 1 ) {
			$('.toggle-docs-private').click();
		}
	}
...
{
	var sVal = 
		($('.private:hidden').length ? "0" : "1");// + '-' +

	createCookie( 'JUI_JSDoc', sVal );
}
